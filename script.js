document.addEventListener('DOMContentLoaded', () => {
    ...
    // Epic Entry Animation for Setup Screen
    gsap.from(".setup-container", {
        y: 100,
        z: -500,
        rotationX: 30,
        opacity: 0,
                ...
                setTimeout(() => {
                    gsap.to(funnySurprise, {
                        ...
                        onComplete: () => {
                            ...
                            // Initialize ScrollTrigger and other listeners after a short delay for DOM layout
                            setTimeout(() => {
    ...
    function escapeNoButton(e) {
        ...
        // Change position to absolute if not already
        if (noBtn.style.position !== 'absolute') {
            const container = document.querySelector('.funny-card');
            
            // Move to card to avoid floating over other sections
        ...
        // Calculate random position within the funny-card
        const container = document.querySelector('.funny-card');
        const btnWidth = noBtn.offsetWidth;
        const btnHeight = noBtn.offsetHeight;
        const safeMargin = 20;
        
        const maxX = container.offsetWidth - btnWidth - safeMargin;
        const maxY = container.offsetHeight - btnHeight - safeMargin;
    ...
    // 5. Memories and General 3D Tilt Effect
    function initTiltEffect() {
        const tiltElements = document.querySelectorAll('.tilt-wrapper, .card, .surprise-box, .polaroid-frame');
        
        tiltElements.forEach(wrapper => {
            // Find the actual element to tilt. For tilt-wrapper it's .memory-card, otherwise it's the wrapper itself.
            const card = wrapper.classList.contains('tilt-wrapper') ? wrapper.querySelector('.memory-card') : wrapper;
            if (!card) return;
            ...
            wrapper.addEventListener('mouseleave', () => {
                ...
                setTimeout(() => card.style.transition = '', 500); // clear transition so mousemove is snappy
            });

            // Heart pop on click (only for memories)
            if (heart) {
    ...
    // 7.5 Surprise Gifts Logic
    const surpriseBoxes = document.querySelectorAll('.surprise-box');
    const revealedGiftContainer = document.getElementById('revealed-gift-container');
    const revealedGiftText = document.getElementById('revealed-gift-text');
    ...
    if (surpriseBoxes.length > 0) {
        surpriseBoxes.forEach(box => {
            box.addEventListener('click', () => {
                ...
                const gift = box.getAttribute('data-gift');
                ...
                gsap.to(box, {
                    ...
                    onComplete: () => {
                        ...
                        // Show revealed gift
                        revealedGiftText.textContent = gift;
                        revealedGiftContainer.classList.remove('hidden');
                        
                        gsap.fromTo(revealedGiftContainer, 
                            {scale: 0.5, opacity: 0, y: 20},
                            {scale: 1, opacity: 1, y: 0, duration: 1, ease: 'back.out(1.5)'}
                        );
                        
                        // Confetti from the revealed area
                        confetti({
                            particleCount: 150,
                            spread: 80,
                            origin: { y: 0.6 }
                        ...
                        // Mark other boxes as disabled
                        surpriseBoxes.forEach(b => {
                            if (b !== box) {
                                gsap.to(b, {opacity: 0.5, scale: 0.9, duration: 0.5});
    ...
    function createLantern() {
        ...
        // Using a glowing circle or star as a lantern if flame is too simple
        lantern.classList.add('fa-solid', 'fa-fire-flame-curved', 'lantern'); 
        ...
        gsap.to(lantern, {
            ...
            duration: Math.random() * 10 + 15, // 15 to 25 seconds for slow float
            ease: 'none',
            onComplete: () => lantern.remove()
        });
    }
    
    // Spawn lantern every 4 seconds
    setInterval(createLantern, 4000);

    // 7. Scroll Reveal & Final Celebration
    function initScrollAnimations() {
        gsap.registerPlugin(ScrollTrigger);

        // General section reveals (Superb 3D)
        const sections = document.querySelectorAll('.hidden-section');
        
        // Ensure CSS perspective is set on mainContent
        gsap.set(document.getElementById('main-content'), { perspective: 1200 });

        sections.forEach(sec => {
            sec.classList.remove('hidden-section'); // Let GSAP handle the hiding/showing
            ...
            gsap.from(sec, {
                scrollTrigger: {
                    ...
                    // markers: true // Un-comment to debug scroll triggers
                },
                opacity: 0,
                y: 100,
                z: -100,
    ...
    function initScratchCard() {
        ...
        ctx.font = 'bold 24px var(--font-body), sans-serif';
        ...
        canvas.addEventListener('mousedown', (e) => { isDrawing = true; const pos = getXY(e); scratch(pos.x, pos.y); });
        canvas.addEventListener('mousemove', (e) => { if (isDrawing) { const pos = getXY(e); scratch(pos.x, pos.y); }});
        canvas.addEventListener('mouseup', () => { isDrawing = false; });
        canvas.addEventListener('mouseleave', () => { isDrawing = false; });
        
        // Touch support
        canvas.addEventListener('touchstart', (e) => { isDrawing = true; const pos = getXY(e); scratch(pos.x, pos.y); e.preventDefault(); }, {passive: false});
        canvas.addEventListener('touchmove', (e) => { if (isDrawing) { const pos = getXY(e); scratch(pos.x, pos.y); e.preventDefault(); }}, {passive: false});
