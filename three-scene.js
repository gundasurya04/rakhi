document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add floating 3D shapes to background for superb 3D effect
    const bgShapes = [];
    const shapeGeometry = new THREE.IcosahedronGeometry(2, 0); // 3D Crystals
    ...
    // Ambient light for the background shapes
    const bgAmbient = new THREE.AmbientLight(0xffffff, 0.5);
    bgScene.add(bgAmbient);
    const bgDirLight = new THREE.DirectionalLight(0xffffff, 1);
    bgDirLight.position.set(10, 10, 10);
    bgScene.add(bgDirLight);
    ...
    function animate() {
        ...
        // 1. Update Background Particles and Camera Parallax
        targetX = mouseX * 0.001;
        targetY = mouseY * 0.001;

        // Subtle 3D parallax on camera
        ...
        // Update floating 3D shapes
        bgShapes.forEach(shape => {
            shape.mesh.rotation.x += shape.rx * 0.5;
            shape.mesh.rotation.y += shape.ry * 0.5;
            shape.mesh.position.y += shape.yVel * 0.5;
            if (shape.mesh.position.y > 70) {
                shape.mesh.position.y = -70;
            }
        });
